...
import { Screen, Scroll, Viewport } from ".";
...
export default class Media {
    ...
    if (this.gl) {
      ...
      this.program = new Program(this.gl, {
        ...
        uniforms: {
          ...
          uViewportSizes: {
            value: [this.viewport?.width ?? 0, this.viewport?.height ?? 0],
          },
          uStrength: { value: 0 },
          uSpeed: { value: 0 },
          uTime: { value: 0 },
  ...
  onResize({
    ...
    this.scale = (this.screen?.height ?? 0) / 550;
  ...
  update(scroll: Scroll, direction?: "right" | "left") {
    if (this.plane) {
      this.plane.position.x = this.x - scroll.current - this.extra;

      const planeOffset = this.plane.scale.x / 2;
      const viewportOffset = this.viewport?.width ?? 0;
      ...
      this.plane.program.uniforms.uStrength.value =
        ((scroll.current - scroll.last) / (this.screen?.width ?? 1)) *
        (isMobile() ? 300 : 2000);
    }
  }
