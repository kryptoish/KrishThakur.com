...
export default class ImageHover {
  ...
  resize(init: boolean = false) {
    ...
    if (this.material) {
      ...
      this.material.uniforms.resolution.value.z = this.img?.naturalWidth ?? 1;
      this.material.uniforms.resolution.value.w = this.img?.naturalHeight ?? 1;
    }

    this.camera.updateProjectionMatrix();
    if ((isMobile() && init) || !isMobile()) this.regenerateGrid();
  }
  ...
  regenerateGrid() {
    ...
    // Hide img element
    this.container?.setAttribute("data-image-loaded", "");
  }
